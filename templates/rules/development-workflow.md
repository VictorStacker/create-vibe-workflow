...
writing-plans         /plan-ceo-review       /opsx:explore        to-prd（对话→PRD）
TDD + subagent-dev    /plan-eng-review       /opsx:archive        grill-with-docs（文档审查）
verification          /plan-design-review    todolist skill       improve-arch（架构审计）
systematic-debugging  /browse + /qa          architecture-gate    caveman（精简）
                      /review + /cso         rules + memory       diagnose（调试）
                      /ship                  hooks + 路由表
                      /context-save
                      /careful /guard
                      /autoplan
...
层5: 计划拆分  → superpowers writing-plans      "拆成几步？"
       🚧 计划审核：/plan-ceo-review + /plan-eng-review + grill-with-docs
...
层8: 审查安全  → arch-gate → /review → /cso     "架构还对吗？代码呢？安全吗？"
       + improve-codebase-architecture（架构审计）+ grill-with-docs（文档审查）
...
| **方案人类确认** | **agent 展示方案 → 等待人类说"可以"（铁律）** | — |
| 写实施计划 | `superpowers writing-plans` | — |
| 生成/更新 todolist | `todolist-management` skill（自动） | — |
| **计划审核（CEO 视角）** | **`gstack /plan-ceo-review`（铁律）** | `/autoplan` |
| **计划审核（工程视角）** | **`gstack /plan-eng-review`（铁律）** | `/autoplan` |
| **计划审核（设计视角）** | **`gstack /plan-design-review`（有 UI 时）** | — |
| **方案文档审查** | **`grill-with-docs` skill（铁律）** | — |
...
| 架构一致性检查 | `architecture-gate` skill（自动） | `improve-codebase-architecture` |
| 架构审计 | `improve-codebase-architecture` skill | — |
| 代码审查 | `gstack /review` | — |
| 安全审查 | `gstack /cso` | — |
...
> **铁律：方案设计完成后，必须先向用户展示方案并等待明确确认，才能进入计划拆分。**
> 未获人类确认 = 禁止进入层5/层6。不可跳过、不可"先写着再说"。

**触发条件** — brainstorming 产出方案后：
- 用清晰易懂的语言（面向 vibe coder）展示：要做什么、怎么做、涉及哪些文件
- 明确请求确认："这个方案可以吗？需要调整吗？"

**agent 自检** — 每次要进入 writing-plans 或写代码前，问自己：
> "我是否已向用户展示了方案？用户是否已明确确认？" → 没有 = 停下来，先展示方案并等待确认

> 这道门禁是对 vibe coder 最重要的保护——确保 AI 理解对了再动手，避免写完才发现方向错了。
...
> **铁律：计划拆分完成后、写代码之前，必须通过 gstack 计划审核 + grill-with-docs 文档审查。**
> 三道审核通过才能进入编码。不可跳过、不可推迟。

**触发条件** — writing-plans 产出 todolist.md 后，依次执行：

| 审核 | 工具 | 审查什么 | 跳过条件 |
|------|------|----------|----------|
| CEO 视角 | `gstack /plan-ceo-review` | 战略/范围是否正确、是否过度设计 | — |
| 工程视角 | `gstack /plan-eng-review` | 架构/数据流/边界情况/测试策略 | — |
| 设计视角 | `gstack /plan-design-review` | UI/UX 设计合理性 | 纯后端/无 UI 项目 |
| 文档审查 | `grill-with-docs` skill | 方案是否覆盖所有需求、无隐藏假设 | — |

> 快捷方式：`/autoplan` 可一键串行跑完以上全部。

**agent 自检** — 每次要加载 TDD skill 开始写代码前，问自己：
> "计划审核是否已通过？grill-with-docs 是否已通过？" → 没有 = 先跑审核

> 这道门禁用 gstack + Matt Pocock 的机器审核替代了 vibe coder 不具备的专业判断——人点头说方向对，机器检查说方案没坑。
...
**目的**：三层审查 + 两个 Matt Pocock 审核技能，从架构到代码到安全，逐层收紧。

- **8a architecture-gate**: 模块边界 / 依赖方向 / 模式一致性（三问检查，自动）
- **8a+ improve-codebase-architecture**: 深度架构审计 — 分析模块深度/接缝（Seam）/依赖方向，给出重构优先级矩阵（P0/P1/P2）
- **8b /review**: SQL 安全性、信任边界、副作用、错误处理
- **8b+ grill-with-docs**: 对照文档审查 — 逐条检查方案是否覆盖所有需求（功能/非功能/约束/边界），发现遗漏、矛盾、未定义假设
...
| 3 | 架构检查 | `architecture-gate` + `improve-codebase-architecture` 已通过 | 立即运行两项 |
| 4 | 安全审查 | `/cso` 已通过（仅 auth/finance/system 模块） | 立即运行 `/cso` |
| 5 | 文档审查 | `grill-with-docs` 已通过 + 无遗漏/矛盾 | 立即运行 `grill-with-docs` |
| 6 | 文档反写 | 改了什么文档就同步了什么 | 补文档 |
| 7 | 进度追踪 | `todolist.md` 中对应任务已勾选 | 更新 todolist |
